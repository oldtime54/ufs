...
function build_iso() {
    ...
    # memtest86
    sudo cp chroot/boot/memtest86+x64.bin image/install/memtest86+
...
menuentry "${GRUB_LIVEBOOT_LABEL}" {
   linux /casper/vmlinuz file=/cdrom/preseed/ubuntu.seed boot=casper nopersistent toram quiet splash ---
...
EOF
	...
	# create preseed
cat <<EOF > image/preseed/ubuntu.seed
